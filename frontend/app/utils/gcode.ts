/**
 * Analyse d'un G-code : extraction des segments d'extrusion.
 *
 * Logique **pure**, sans DOM ni worker, pour être testable directement. Le
 * worker `gcode.worker.ts` ne fait que l'appeler hors du thread principal.
 */

/** Nombre maximal de segments conservés par défaut (au-delà, sous-échantillonnage). */
 ...
 *
 * Gère les modes absolus/relatifs des déplacements (`G90`/`G91`) et de
 * l'extrusion (`M82`/`M83`), ainsi que la remise à zéro `G92`. Les mouvements à
 * vide (sans extrusion) ne sont pas retenus.
 *
 * `maxSegments` borne le nombre de segments conservés : au-delà, un
 * sous-échantillonnage régulier est appliqué.
 */
export function parseGcode(text: string, maxSegments: number = MAX_SEGMENTS): ParsedGcode {
  ...
  while (lineStart < len) {
    ...
    if (extruding && hasPrev) {
      ...
      if (oz > maxZ) maxZ = oz
      if (z > maxZ) maxZ = z
      if (Number.isNaN(lastLayerZ) || Math.abs(z - lastLayerZ) > 1e-3) {
        layers++
        lastLayerZ = z
      }
    }
  ...
  const total = n / 6
  const step = total > maxSegments ? Math.ceil(total / maxSegments) : 1
